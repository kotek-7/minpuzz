...
describe('pieceService.place (click)', () => {
  test('success without holder/lock; sets placed and row/col', async () => {
    ...
    const pieceId = 'p1';
    await store.setPiece(matchId, { id: pieceId, x: 0, y: 0, placed: false } as any);
    const r = await PieceService.place(store as any, { matchId, pieceId, userId: 'u1', row: 1, col: 2, x: 0, y: 0 });
  ...
  test('rejects out-of-range cell', async () => {
    ...
    const r = await PieceService.place(store as any, { matchId, pieceId: 'p1', userId: 'u1', row: 5, col: 0, x: 0, y: 0 });
  ...
  test('rejects when cell is already occupied', async () => {
    ...
    await store.setPiece(matchId, { id: 'p1', x: 0, y: 0, placed: false } as any);
    await store.setPiece(matchId, { id: 'p2', x: 0, y: 0, placed: true, row: 0, col: 0 } as any);
    const r = await PieceService.place(store as any, { matchId, pieceId: 'p1', userId: 'u1', row: 0, col: 0, x: 0, y: 0 });
    expect(r.isErr() && r.error === 'invalidCell').toBe(true);
  });
  ...
  test('rejects already placed piece', async () => {
    ...
    await store.setPiece(matchId, { id: 'p1', x: 0, y: 0, placed: true, row: 1, col: 1 } as any);
    const r = await PieceService.place(store as any, { matchId, pieceId: 'p1', userId: 'u1', row: 1, col: 2, x: 0, y: 0 });
