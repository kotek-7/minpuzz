...
## ユニット6: ソケット（JOIN_GAME/REQUEST_GAME_INIT） [ ]
- 目的: 初期化の往復とタイマー開始まで。
- 対象ファイル:
...
## ユニット7: ソケット（PIECE_PLACE → placed/progress/end） [ ]
- 目的: 配置に伴う通知連鎖と終了の一回性。
- 対象ファイル:
  - `backend/src/socket/teamSocket.ts`
- 実装要点:
